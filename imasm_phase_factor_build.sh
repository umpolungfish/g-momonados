...
RUSTFLAGS='-D warnings' cargo build --bin godel >/dev/null
IMASM_N_WORD="$(./target/debug/godel encode "$IMASM_N_DECIMAL" | awk '$1 == "word" {print $2}')"
IMASM_BASE_WORD="$(./target/debug/godel encode "$IMASM_BASE_DECIMAL" | awk '$1 == "word" {print $2}')"
...
IMASM_PHASE_N_WORD="$IMASM_N_WORD" IMASM_PHASE_BASE_WORD="$IMASM_BASE_WORD" \
  RUSTFLAGS='-D warnings' cargo build --bin imasm_phase_factor >/dev/null
mkdir -p "$(dirname "$IMASM_OUTPUT")"
cp target/debug/imasm_phase_factor "$IMASM_OUTPUT"
