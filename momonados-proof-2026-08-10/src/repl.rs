    ...
    loop {
        ...
        match cmd {
            ...
            "native_numeral" | "numeral" => {
                use imasm_core::numeral_mapping as nn;
                let sub = parts.next().unwrap_or("");
                match sub {
                    "" | "help" => {
                        sprintln!("native_numeral — IMASM numeral mapping (Grammar-native)");
                        sprintln!("  native_numeral word         — print the canonical decoder word");
                        sprintln!("  native_numeral encode <n>   — encode decimal N to direct numeral word D(N)");
                        sprintln!("  native_numeral factor <W>   — Λ(W_N) deinterlace: extract (p,q) from D(N)");
                        sprintln!("  native_numeral verify <n>   — run canonical decoder on N, show trace");
                    }
                    "word" => {
                        sprintln!("{}", nn::CANONICAL_MAPPING_WORD);
                    }
                    "encode" => {
                        let n_str = parts.next().unwrap_or("");
                        if n_str.is_empty() {
                            sprintln!("native_numeral encode: usage: native_numeral encode <n>");
                        } else if let Ok(n) = n_str.parse::<i64>() {
                            sprintln!("{}", nn::encode_report(n));
                        } else {
                            sprintln!("native_numeral encode: invalid integer '{}'", n_str);
                        }
                    }
                    "factor" | "deinterlace" => {
                        let word = parts.next().unwrap_or("");
                        if word.is_empty() {
                            sprintln!("native_numeral factor: usage: native_numeral factor <direct-numeral-word>");
                        } else {
                    ...
                    "verify" => {
                        let n_str = parts.next().unwrap_or("");
                        if n_str.is_empty() {
                            sprintln!("native_numeral verify: usage: native_numeral verify <n>");
                        } else if let Ok(n) = n_str.parse::<i64>() {
                            let result = nn::run_canonical_mapping(n);
                            sprintln!("Canonical decoder on N = {}", n);
                            sprintln!("Word: {}", result.canonical_word);
                            sprintln!("Closed: {}", result.closed);
                            sprintln!("Final register: {}", result.final_register);
                            sprintln!("Final magnitude: {}", result.final_magnitude);
                            sprintln!("Final value: {:?}", result.final_value);
                            sprintln!("Final parity: {:?}", result.final_parity);
                            sprintln!();
                            sprintln!("Trace:");
                            for (i, step) in result.history.iter().enumerate() {
                                sprintln!("  {} {} {} {} {} → {}", 
                                    i + 1, step.glyph, step.opcode, 
                                    step.action, step.reg_before, step.reg_after);
                            }
                        } else {
                            sprintln!("native_numeral verify: invalid integer '{}'", n_str);
                        }
                    }
                    other => {
                        sprintln!("native_numeral: unknown subcommand '{}'", other);
                        sprintln!("Try: native_numeral help");
                    }
                }
            },
            "proof" => {
