// Copied from G-mOMonadOS/src/abc_iutt.rs; hosted Vox membrane port.
#![allow(dead_code)]
...
/// The outer frame holds a single scan. Requested cutoffs read its prefix
/// maxima, including repeated or descending cutoffs, without rerunning it.
pub struct WindowMembrane {
    scan: Vec<(u64, f64, Triple)>,
}

impl WindowMembrane {
    pub fn new(eps: f64, cutoffs: &[u64]) -> Self {
        let max_c = cutoffs.iter().copied().max().unwrap_or(0);
        Self { scan: incremental_window_scan(eps, max_c) }
    }

    pub fn at(&self, cutoff: u64) -> Option<(f64, Triple)> {
        let index = usize::try_from(cutoff.checked_sub(2)?).ok()?;
        self.scan.get(index).map(|&(_, value, triple)| (value, triple))
    }
}
...
mod tests {
    ...
    #[test]
    fn membrane_matches_independent_windows_in_requested_order() {
        for eps in [0.0, 0.1, 0.5, 1.0] {
            let cutoffs = [0, 1, 2, 9, 32, 81, 200, 9, 2];
            let membrane = WindowMembrane::new(eps, &cutoffs);
            for cutoff in cutoffs {
                assert_eq!(membrane.at(cutoff), window_maximum(eps, cutoff));
            }
        }
        assert!(WindowMembrane::new(0.1, &[]).at(2).is_none());
    }

    #[test]
    fn membrane_timing_control() {
        let cutoffs = [200, 400, 600, 800, 1000];
        let eps = std::hint::black_box(0.1);
        let start = std::time::Instant::now();
        let reference: Vec<_> = cutoffs.iter().map(|&c| window_maximum(eps, c)).collect();
        let reference_time = start.elapsed();
        let start = std::time::Instant::now();
        let membrane = WindowMembrane::new(eps, &cutoffs);
        let nested: Vec<_> = cutoffs.iter().map(|&c| membrane.at(c)).collect();
        let nested_time = start.elapsed();
        assert_eq!(nested, reference);
        println!("abc cutoffs={cutoffs:?} reference={reference_time:?} nested={nested_time:?}");
    }
