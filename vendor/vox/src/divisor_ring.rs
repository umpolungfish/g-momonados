// Copied from G-mOMonadOS/src/divisor_ring.rs; hosted Vox membrane port.
#![allow(dead_code)]
...
pub fn divisors(n: u64) -> Vec<u64> {
    ...
    let mut result = divisors_from_factors(&prime_factors(n));
    result.sort_unstable();
    result
}

#[cfg(test)]
fn divisors_reference(n: u64) -> Vec<u64> {
...
// Each prime exponent encloses the combinations of the remaining primes.
// This directly produces the exponent-lexicographic order used by the ring.
fn divisors_from_factors(factors: &[(u64, u32)]) -> Vec<u64> {
    let mut values = vec![1];
    for &(p, exponent) in factors.iter().rev() {
        let inner = values;
        values = Vec::with_capacity(inner.len() * (exponent as usize + 1));
        let mut power = 1;
        for e in 0..=exponent {
            values.extend(inner.iter().map(|&d| d * power));
            if e < exponent { power *= p; }
        }
    }
    values
}
...
pub fn ring_spectrum(k: usize) -> RingSpectrum {
    if k < 2 {
        RingSpectrum { rho: 0.0, gap: 0.0, energy: 0.0, spectrum: vec![0.0; k] }
    } else if k == 2 {
        RingSpectrum {
            rho: 1.0,
            gap: 0.0,
...
pub fn analyze(n: u64) -> DivisorRingResult {
    let ordered = if n == 0 { Vec::new() } else {
        divisors_from_factors(&prime_factors(n))
    };
    let mut divs = ordered.clone();
    divs.sort_unstable();
    let k = divs.len();
    ...
    if k <= 1 {
        return DivisorRingResult {
            ...
            omega: Some(0),
            ring: None,
            note: format!("The input {n} has no nontrivial divisor ring."),
        };
    }
...
mod membrane_tests {
    use super::*;
    #[test]
    fn generated_ring_matches_trial_divisors_and_exponent_sort() {
        for n in 1..=2000 {
            let expected = divisors_reference(n);
            let actual = analyze(n);
            assert_eq!(actual.sigma_n, expected, "n={n}");
            if let Some(ring) = actual.ring {
                assert_eq!(ring.units, divisor_lattice_chain(&expected), "n={n}");
            }
        }
        assert_eq!(analyze(0).k, 0);
        assert_eq!(analyze(1).k, 1);
    }

    #[test]
    fn membrane_timing_control() {
        let n = std::hint::black_box(1u64 << 50);
        let start = std::time::Instant::now();
        let expected = divisors_reference(n);
        let reference = start.elapsed();
        let start = std::time::Instant::now();
        let result = analyze(n);
        let nested = start.elapsed();
        assert_eq!(result.sigma_n, expected);
        assert_eq!(result.k, 51);
        println!("divisor n={n} reference_divisors={reference:?} nested_full_analysis={nested:?}");
    }
}
