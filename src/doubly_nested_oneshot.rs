...
//
// One level deeper than nested_oneshot. The nested_oneshot payload is enclosed
// in an outer hold frame, its ⊢/⊣ interfaces and terminal ⊡ stripped so the
// payload interior abuts the outer fuse (a proper nested word):
//   ⊢∈⊤∈≻⊤≺⊥⋈⊙⊞∋∋⊡⊣
//   outer ⊢∈⊤ · payload interior ∈≻⊤≺⊥⋈⊙⊞∋ · outer ∋⊡⊣
//
// Period: 15 (kernel-verified via imasm cycle).
...
pub fn repl_doubly_nested_oneshot(args: &[&str]) {
    if args.is_empty() || args[0] == "help" {
        ...
        sprintln!("  Word: ⊢∈⊤∈≻⊤≺⊥⋈⊙⊞∋∋⊡⊣  (period 15, 5 distinct landings)");
