...
pub fn repl(k: &mut Kernel) {
    repl_seeded(k, alloc::collections::VecDeque::new())
}

/// Same REPL, with `seed` run first as if typed at the prompt — every command
/// reachable non-interactively this way, with no OS-level stdin pipe needed.
/// Once `seed` drains, behavior is identical to `repl`: falls through to real
/// `read_line`. A seed ending in `quit`/`exit`/`halt` returns before that,
/// so a fully-seeded run never touches stdin at all.
pub fn repl_seeded(k: &mut Kernel, seed: alloc::collections::VecDeque<String>) {
    ...
    let mut pending_lines: alloc::collections::VecDeque<String> = seed;
        ...
        match cmd {
            ...
            "imrun" => {
                ...
                if rest.is_empty() {
                    ...
                    match std::fs::read(file) {
                        Ok(raw) => {
            // A glyph-only module (a `.glyphs` file, or one whose head is the
            // glyph prefix) is decoded back to the executable module first: the
            // kernel boots a program carried in nothing but the twelve marks.
                            let is_glyph = file.ends_with(".glyphs")
                                || raw.starts_with(vox_core::glyph_module::PREFIX.as_bytes());
                            let module: Option<String> = if file.ends_with(".imasm") {
                                Some(String::from_utf8_lossy(&raw).into_owned())
                            } else if is_glyph {
                                match vox_core::glyph_module::decode(&String::from_utf8_lossy(&raw)) {
                                    Ok(m) => Some(m),
                                    Err(e) => {
                                        sprintln!("[imrun] not a valid glyph module: {}", e);
                                        None
                                    }
                                }
                            } else {
                                Some(crate::imasm_exec::emit(&raw))
                            };
            ...
            "gpu_shor" => {
                ...
                if sub == "verify" {
                    ...
                    // Any length, no caps: decimal tapes in. u64-fitting
                    // pairs take the GPU lane; everything else the uncapped
                    // tape orbit (closes within phi(n) <= n steps, always).
                    let toks: Vec<&str> = line.split_whitespace().collect();
                    if toks.len() < 4 {
                        sprintln!("gpu_shor order <a> <n> -- a and n take decimal integers of any length");
                    } else {
                        let a_str = toks[2];
                        let n_str = toks[3];
                        match (vox_core::morphism_factor::decimal_to_tape(a_str), vox_core::morphism_factor::decimal_to_tape(n_str)) {
                            (Some(_), Some(n)) if vox_core::morphism_factor::zero(&n) => {
                                sprintln!("gpu_shor order: zero modulus -- boundary undefined");
                            }
                            (Some(a), Some(n)) => match (a_str.parse::<u64>(), n_str.parse::<u64>()) {
                                (Ok(a64), Ok(n64)) => match crate::gpu_shor::order(a64, n64) {
                                    Some(r) => {
                                        sprintln!("gpu_shor order: order of {} mod {} = {} (gpu lane)", a64, n64, r);
                                        let r_t = vox_core::morphism_factor::tape_u64(r);
                                        let (f, reason) = crate::gpu_shor::factor_close(&a, &n, &r_t);
                                        match f {
                                            Some((p, q)) => {
                                                let prod = vox_core::morphism_factor::trim(vox_core::morphism_factor::mul(&p, &q));
                                                sprintln!("gpu_shor factor: factors recovered via gcd: {} * {} = {} (verified: {})",
                                                    vox_core::morphism_factor::dec_of(&p), vox_core::morphism_factor::dec_of(&q),
                                                    vox_core::morphism_factor::dec_of(&prod), prod == n[..]);
                                            }
                                            None => sprintln!("gpu_shor factor: no factors this base ({}); the order r = {} is still exact", reason, r),
                                        }
                                    }
                                    None => {
                                        let r_t = crate::gpu_shor::order_tape(&a, &n);
                                        sprintln!("gpu_shor order: order of {} mod {} = {} (tape lane; gpu absent)", a64, n64,
                                            r_t.as_ref().map(|r| vox_core::morphism_factor::dec_of(r)).unwrap_or_else(|| "undefined".into()));
                                        if let Some(r) = &r_t {
                                            let (f, reason) = crate::gpu_shor::factor_close(&a, &n, r);
                                            match f {
                                                Some((p, q)) => {
                                                    let prod = vox_core::morphism_factor::trim(vox_core::morphism_factor::mul(&p, &q));
                                                    sprintln!("gpu_shor factor: factors recovered via gcd: {} * {} = {} (verified: {})",
                                                ...
                                                None => sprintln!("gpu_shor factor: no factors this base ({}); the order r = {} is still exact",
                                                    reason, vox_core::morphism_factor::dec_of(r)),
                                            }
                                        }
                                    }
                                },
                                _ => {
                                    let r_t = crate::gpu_shor::order_tape(&a, &n);
                                    sprintln!("gpu_shor order: order of {} mod {} = {} (tape lane)", a_str, n_str,
                                        r_t.as_ref().map(|r| vox_core::morphism_factor::dec_of(r)).unwrap_or_else(|| "undefined (zero modulus)".into()));
                                    if let Some(r) = &r_t {
                                        let (f, reason) = crate::gpu_shor::factor_close(&a, &n, r);
                                        match f {
                                            Some((p, q)) => {
                            ...
                            _ => sprintln!("gpu_shor order: tokens must be plain decimal digits (any length)"),
                        }
                    }
                } else if sub == "bsgs" {
                    // Any length, no caps: u64 pairs whose GPU baby table
                    // fits memory (n <= 2.56e14) take the GPU BSGS lane;
                    // everything else the tape BSGS lane -- one memory-sized
                    // baby table, giants walk to ceil(n/m0), the first
                    // maximal-j collision is exactly the minimal exponent.
                    let toks: Vec<&str> = line.split_whitespace().collect();
                    if toks.len() < 4 {
                        sprintln!("gpu_shor bsgs <a> <n> -- a and n take decimal integers of any length");
                    } else {
                        let a_str = toks[2];
                        let n_str = toks[3];
                        match (vox_core::morphism_factor::decimal_to_tape(a_str), vox_core::morphism_factor::decimal_to_tape(n_str)) {
                            (Some(_), Some(n)) if vox_core::morphism_factor::zero(&n) => {
                                sprintln!("gpu_shor bsgs: zero modulus -- boundary undefined");
                            }
                            (Some(a), Some(n)) => match (a_str.parse::<u64>(), n_str.parse::<u64>()) {
                                (Ok(a64), Ok(n64)) if n64 <= 256_000_000_000_000 => match crate::gpu_shor::order_bsgs(a64, n64, u64::MAX) {
                                    Some(r) => sprintln!("gpu_shor bsgs: order of {} mod {} = {} (gpu lane)", a64, n64, r),
                                    None => sprintln!("gpu_shor bsgs: order of {} mod {} = {} (tape lane; gpu absent)", a64, n64,
                                        crate::gpu_shor::order_bsgs_tape(&a, &n).map(|r| vox_core::morphism_factor::dec_of(&r)).unwrap_or_else(|| "undefined".into())),
                                },
                                _ => sprintln!("gpu_shor bsgs: order of {} mod {} = {} (tape lane)", a_str, n_str,
                                    crate::gpu_shor::order_bsgs_tape(&a, &n).map(|r| vox_core::morphism_factor::dec_of(&r)).unwrap_or_else(|| "undefined (zero modulus)".into())),
                            },
                            _ => sprintln!("gpu_shor bsgs: tokens must be plain decimal digits (any length)"),
                        }
                    }
                } else if sub == "fde_step" {
                    // One FDE carrier step: state*base mod n with state = a,
                    // so this reports a^2 mod n -- a single step cannot carry
                    // the order; that is what order/bsgs compute. GPU lanes
                    // first; the CPU mirror answers on tapes at any length.
                    let toks: Vec<&str> = line.split_whitespace().collect();
                    if toks.len() < 4 {
                        sprintln!("gpu_shor fde_step <a> <n> -- a and n take decimal integers of any length");
                    } else {
                        let a_str = toks[2];
                        let n_str = toks[3];
                        match (vox_core::morphism_factor::decimal_to_tape(a_str), vox_core::morphism_factor::decimal_to_tape(n_str)) {
                            (Some(_), Some(n)) if vox_core::morphism_factor::zero(&n) => {
                                sprintln!("gpu_shor fde_step: zero modulus -- boundary undefined");
                            }
                            (Some(a), Some(n)) => {
                                let gpu = crate::gpu_shor::fde_step(&a, &a, &n);
                                let cpu = crate::gpu_shor::fde_step_tape(&a, &a, &n);
                                match (gpu, cpu) {
                                    (Some(g), Some(c)) if g == c => sprintln!("gpu_shor fde_step: {}^2 mod {} = {} (gpu lanes and cpu mirror agree)", a_str, n_str, vox_core::morphism_factor::dec_of(&c)),
                                    (Some(g), _) => sprintln!("gpu_shor fde_step: {}^2 mod {} = {} (gpu lanes; cpu mirror diverged)", a_str, n_str, vox_core::morphism_factor::dec_of(&g)),
                                    (None, Some(c)) => sprintln!("gpu_shor fde_step: {}^2 mod {} = {} (cpu mirror; gpu absent or diverged)", a_str, n_str, vox_core::morphism_factor::dec_of(&c)),
                                    (None, None) => sprintln!("gpu_shor fde_step: divergence in both lanes"),
                                }
                            }
                            _ => sprintln!("gpu_shor fde_step: tokens must be plain decimal digits (any length)"),
                        }
                    }
                } else {
                    sprintln!("gpu_shor order <a> <n> | bsgs <a> <n> [step_cap] | fde_step <a> <n> | verify  -- multiplicative order (Shor period) on GPU; args past u64 run the arbitrary-size tape search (step-capped, compute-bound)");
                }
            }
            "gpu_opi" => {
                let sub = parts.next().unwrap_or("");
            ...
            "gpu_os" => {
                ...
                match sub {
                    "run" => {
                        let word: String = parts.collect::<Vec<&str>>().join("");
                        if word.is_empty() {
                            sprintln!("gpu_os run <glyph-word>  -- run a word through the resident OS kernel on the GPU");
                        } else {
                            sprintln!("{}", crate::gpu_os::run(&word, 0));
                        }
                    }
                    _ => sprintln!("gpu_os run <glyph-word>  -- resident-kernel exec, checked against the CPU control"),
                }
            }
            "gpu_kernel" => {
                let sub = parts.next().unwrap_or("help");
                match sub {
                    ...
                    "factor" => {
                        // No size cap, no external ceiling: N of any width factors
                        // here, and the sweep bounds itself by N's own sqrt. Inputs
                        // all below 2^64 take the fixed-width path (which reaches the
                        // full square root); anything larger takes the self-bounded
                        // multi-limb path.
                        let toks: Vec<String> = parts.collect::<Vec<&str>>().join(" ")
                            .split_whitespace().map(|s| s.to_string()).collect();
                        if toks.is_empty() { sprintln!("gpu_kernel factor <n> [n ...]  -- any width; the sweep bounds itself by sqrt(N)"); }
                        else {
                            let mut bigs: Vec<num_bigint::BigUint> = Vec::new();
                            let mut bad: Option<String> = None;
                            for s in &toks {
                                match s.parse::<num_bigint::BigUint>() { Ok(v) => bigs.push(v), Err(_) => { bad = Some(s.clone()); break; } }
                            }
                            match bad {
                                Some(s) => sprintln!("gpu_kernel factor: '{}' is not a non-negative integer", s),
                                None => {
                                    let all_u64 = bigs.iter().all(|b| b.to_u64_digits().len() <= 1);
                                    if all_u64 {
                                        let nums: Vec<u64> = bigs.iter().map(|b| b.to_u64_digits().first().copied().unwrap_or(0)).collect();
                                        sprintln!("{}", crate::gpu_kernel::bench_factor(&nums, 0));
                                    } else {
                                        sprintln!("{}", crate::gpu_kernel::bench_factor_big(&bigs, 0));
                                    }
                                }
                            }
                        }
            ...
            "shor-qft" => {
                // Re-tokenize: parts is splitn(4,' '), so the last chunk can
                // carry two glued numbers ("21 8"); joining and re-splitting
                // on whitespace gives repl_shor_qft one token per number.
                // (Same class as the gpu_shor arms' splitn swallow.)
                let tail: Vec<&str> = parts.collect();
                let rest: Vec<String> = tail.join(" ").split_whitespace().map(|s| s.to_string()).collect();
                let args: Vec<&str> = rest.iter().map(|x| x.as_str()).collect();
            ...
            "shor_b4" | "shor_b4_membrane" | "key_membrane" => {
                let tail: Vec<&str> = parts.collect();
                let rest: Vec<String> = tail.join(" ").split_whitespace().map(|s| s.to_string()).collect();
                let args: Vec<&str> = rest.iter().map(|x| x.as_str()).collect();
                sprintln!("{}", crate::shor_b4_membrane::repl_shor_b4_membrane(&args));
            }
            "membrane_family" | "mfam" => {
                let tail: Vec<&str> = parts.collect();
                let rest: Vec<String> = tail.join(" ").split_whitespace().map(|s| s.to_string()).collect();
                let args: Vec<&str> = rest.iter().map(|x| x.as_str()).collect();
                sprintln!("{}", crate::membrane_family::repl_membrane(&args));
            }
            "coupled_bridge" | "cbridge" | "bridge" => {
                let tail: Vec<&str> = parts.collect();
                let rest: Vec<String> = tail.join(" ").split_whitespace().map(|s| s.to_string()).collect();
                let args: Vec<&str> = rest.iter().map(|x| x.as_str()).collect();
                sprintln!("{}", crate::coupled_bridge::repl_coupled_bridge(&args));
            }
            ...
            "factor_big" | "bigfactor" => {
                let n_str = parts.next().unwrap_or("");
                let tries: u32 = parts.next().and_then(|t| t.parse().ok()).unwrap_or(16);
                let step_cap: u64 = parts.next().and_then(|t| t.parse().ok()).unwrap_or(1_000_000);
                if n_str.is_empty() {
                    sprintln!("factor_big: usage: factor_big <N> [tries=16] [step_cap=1000000]");
                } else {
                    sprintln!("{}", crate::winding_period::repl_factor_big_call(n_str, tries, step_cap));
                }
            }
            ...
            "native_numeral" | "numeral" => {
                ...
                match sub {
                    ...
                    "decode" => {
                        ...
                        if word.is_empty() {
                            sprintln!("native_numeral decode: usage: native_numeral decode <word>");
                        } else {
                            sprintln!("{}", nn::decode_report(word));
                        }
                    }
                    "factor" => {
...
fn trilattice_dialect_probe(n_str: &str, a_str: &str, max_k: Option<u64>) -> String {
    ...
    // DECOMPOSITION READOUT: every dialect's own gate-pass period, grouped by
    // value, with whether each divides the true order. If the order factors as
    // a join (LCM) of short per-dialect periods, the divisors show up here as
    // cheap periods that no dialect had to walk the full order to reach — the
    // Pohlig-Hellman shape rendered as a dialect consensus, testable directly.
    let mut by_period: std::collections::BTreeMap<u64, Vec<u8>> = std::collections::BTreeMap::new();
    for &(u, p) in &periods {
        by_period.entry(p).or_default().push(u);
    }
    out.push_str(&format!("per-dialect gate-pass periods ({} distinct):\n", by_period.len()));
    for (p, us) in &by_period {
        let divides = match true_order {
            Some(r) => if r % p == 0 { if *p == r { "= order".to_string() } else { format!("| order (order/{} = {})", p, r / p) } } else { "does NOT divide order".to_string() },
            None => "order unknown".to_string(),
        };
        out.push_str(&format!("  period {:>6}  x{:>2} dialect(s) {:?}  {}\n", p, us.len(), us, divides));
    }
    if let Some(r) = true_order {
        // The lemma's live question: is the order the LCM of the proper-divisor
        // periods alone? If yes, the full-order dialects are redundant — the
        // order is reachable by combining short periods, never walking to r.
        let proper: Vec<u64> = by_period.keys().cloned().filter(|&p| p > 1 && p < r && r % p == 0).collect();
        let lcm_proper = proper.iter().fold(1u64, |acc, &p| {
            let g = { let (mut x, mut y) = (acc, p); while y != 0 { let t = x % y; x = y; y = t; } x };
            acc / g * p
        });
        out.push_str(&format!("LCM of proper-divisor periods {:?} = {}  (true order = {}, reconstructed-without-walking: {})\n",
            proper, lcm_proper, r, lcm_proper == r));
    }
    out
}
