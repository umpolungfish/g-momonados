...
const DIVISOR_REACH: u64 = u64::MAX;
...
/// Integer square root of a BigUint, searching the full integer square root range.
fn search_bound(n: &BigUint) -> u64 {
    // Native Newton on limbs: machine speed at any width.
    if n < &BigUint::from(4u32) { return 2; }
    let mut x = BigUint::one() << ((n.bits() + 1) / 2); // overestimate
    loop {
        let y = (&x + n / &x) >> 1u32;
        if y >= x { break; }
        x = y;
    }
    x.iter_u64_digits().next().unwrap_or(u64::MAX)
}
...
/// Word-native Miller-Rabin, any width: square-and-multiply modpow over
/// multiply_via_word/modulo_via_word, even-extraction via divmod_small_via_word.
/// Twelve fixed bases; deterministic below 3.3e24, error < 4^-12 above.
fn is_prime_mr(n: &BigUint) -> bool {
    // Native BigUint ops (machine schoolbook on limbs) -- the word-op version
    // is quadratic in tape length and cannot carry 20k-bit inputs. The
    // Grammar's tape verify surface (vox verify) closes any reported pair;
    // this gate only has to be fast and correct.
    if n < &BigUint::from(2u32) { return false; }
    let one = BigUint::one();
    let zero = BigUint::zero();
    let small: &[u64] = if n.bits() > 4096 { &[2, 3] } else { &[2, 3, 5, 7, 11, 13] };
    for &p in small {
        let pb = BigUint::from(p);
        if n == &pb { return true; }
        if n % &pb == zero { return false; }
    }
    let mut d = n - &one;
    let mut s = 0u32;
    while (&d % 2u32) == zero { d >>= 1u32; s += 1; }
    let n_minus_1 = n - &one;
    for &a in small {
        let ab = BigUint::from(a);
        if &ab >= n { continue; }
        let mut acc = BigUint::modpow(&ab, &d, n);
        if acc == one || acc == n_minus_1 { continue; }
        let mut composite = true;
        for _ in 0..s {
            acc = &acc * &acc % n;
            if acc == n_minus_1 { composite = false; break; }
        }
...
fn split_full(g: &Gpu, n: BigUint, out: &mut Vec<BigUint>) -> Result<(), String> {
    ...
    // Trial division first: a small factor costs one GPU sweep, while a full
    // primality gate on a huge n costs modpows over every limb. Prime check
    // only runs when the small sweep finds nothing.
