...
mod tests {
    ...
    #[test]
    fn b4_schoolbook_mul_preserves_balanced_carry_parity() {
        // P = 13 (1101_2 LSB), Q = 11 (1011_2 LSB), N = 143 (10001111_2 LSB)
        let p_cells = vec![B4::T, B4::F, B4::T, B4::T];
        let q_cells = vec![B4::T, B4::T, B4::F, B4::T];
        let prod = b4_schoolbook_mul(&p_cells, &q_cells);
        let digits = b4_digit_channel(&prod);
        let n_val: u32 = digits.iter().enumerate().fold(0, |acc, (i, &v)| {
            acc | (if v == B4::T { 1 << i } else { 0 })
        });
        assert_eq!(n_val, 143);
        // Parity channel yields gamma: [0, 0, 0, 0, 1, 1, 1, 1]
        let gammas: Vec<bool> = prod.iter().map(|&v| v.to_wh2().1).collect();
        assert_eq!(&gammas[..8], &[false, false, false, false, true, true, true, true]);
    }
}
