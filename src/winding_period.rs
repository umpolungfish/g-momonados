...
/// REPL wrapper for factor_big: BigUint Shor, no exponential phase register.
pub fn repl_factor_big(n_str: &str, max_tries: u32, step_cap: u64) -> String {
    use num_bigint::BigUint;
    let n: BigUint = match n_str.trim().parse() {
        Ok(v) => v,
        Err(_) => return format!("'{}' is not a decimal integer", n_str),
    };
    let r = factor_big(&n, max_tries, step_cap, 0xC0FFEE_5EED_BEEF_u64);
    let mut o = String::new();
    o.push_str("factor_big (BigUint Shor — baby-step/giant-step order, no exponential RAM)\n");
    o.push_str(&format!("N = {} ({} bits)\n", n, n.bits()));
    match r {
        FactorBig::Found { a, r, p, q } => {
            o.push_str(&format!("a = {}, order r = {}\n", a, r));
            o.push_str(&format!("FACTORS:\np = {}\nq = {}\n", p, q));
            o.push_str(&format!("p × q = N: {}\n", &p * &q == n));
        }
        FactorBig::OutOfReach => o.push_str("OutOfReach — order > step_cap; raise step_cap and retry\n"),
        FactorBig::NoFactorInTries => o.push_str(&format!("NoFactorInTries — {} bases tried, none yielded an even order with a^(r/2) not ±1\n", max_tries)),
    }
    o
}
...
/// REPL/CLI bridge for factor_big.
pub fn repl_factor_big_call(n_str: &str, tries: u32, step_cap: u64) -> String {
    repl_factor_big(n_str, tries, step_cap)
}

/// CLI args parser.
pub fn repl_factor_big_wiring(args: &[&str]) -> String {
    if args.is_empty() { return "usage: factor_big <N> [tries=16] [step_cap=1000000]".into(); }
    let n_str = args[0];
    let tries: u32 = args.get(1).and_then(|s| s.parse().ok()).unwrap_or(16);
    let step_cap: u64 = args.get(2).and_then(|s| s.parse().ok()).unwrap_or(1_000_000);
    repl_factor_big(n_str, tries, step_cap)
}
#[cfg(test)]
mod big_tests {
