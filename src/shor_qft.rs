...
//! but never derives that phase from an amplitude register.
//!
//! This file does the missing half: a real complex-amplitude run
...
//! and YZ's separation is titled to prove it doesn't need either. The amplitude register costs O(M^2) in the register size M=2^n --
//! exponential in the qubit count -- which is exactly where an amplitude
//! device would have a genuine advantage at a scale this register
//! cannot reach; the small scale here is what makes the run
...
pub fn repl_shor_qft(args: &[&str]) {
    if args.is_empty() || args[0] == "help" {
        sprintln!("shor-qft — Shor's period-finding on a real complex-amplitude statevector register");
    ...
    match args[0] {
        "run" => {
            ...
            match run_shor_register(a, n_val, qubits) {
