...
pub(crate) const SDEPTH: usize = 256;  // stack depth
pub(crate) const META: usize = 32;     // Classical state followed by the banked tri face.
...
const KERNEL_SRC: &str = concat!(include_str!("gpu_imasm_engine.cuh"), r#"
    ...
    unsigned long long max_ticks, unsigned int n)
{
    ...
    const int STRIDE = PROGRAM_CAP, SDEPTH = 256, META = 32;
    ...
    imasm_exec_one(P, plen, max_ticks, &out_stack[gid*SDEPTH], &out_meta[gid*META]);
}
"#);
...
// Multi-limb trial division: N held as little-endian 64-bit limbs, so there is
// no 64-bit size cap on the input. Trial division needs only N mod d for a
// small candidate d, and N mod d is a Horner fold over the limbs from the top
// down, each step ((r<<64)|limb) % d carried in a 128-bit intermediate. The
// candidate d itself stays a u64, so the __int128 modulo is exact. The square
// root of a large N is unreachable, so the sweep is bounded by a divisor
// ceiling dceil instead of d*d<=N: this finds every prime factor of N below
// dceil, of an N of any width, and reports none within the ceiling otherwise.
const KERNEL_FACTOR_ML_SRC: &str = r#"
// a*b mod m by double-and-add, every intermediate under 2m, valid for m below
// 2^63 — no 128-bit type, which NVRTC gates behind a build flag.
__device__ __forceinline__ unsigned long long mm(unsigned long long a, unsigned long long b, unsigned long long m) {
    unsigned long long r = 0ULL; a %= m;
    while (b) { if (b & 1ULL) { r += a; if (r >= m) r -= m; } a += a; if (a >= m) a -= m; b >>= 1ULL; }
    return r;
}
extern "C" __global__ void run_factor_ml(
    const unsigned long long* limbs, unsigned int n_limbs,
    unsigned long long* out, unsigned int k, unsigned int tpn,
    unsigned long long dstart, unsigned long long dend)
{
    unsigned long long gid = (unsigned long long)blockIdx.x * blockDim.x + threadIdx.x;
    unsigned long long total = (unsigned long long)k * tpn;
    if (gid >= total) return;
    unsigned int lane = (unsigned int)(gid / tpn);
    unsigned int t    = (unsigned int)(gid % tpn);
    const unsigned long long* N = limbs + (unsigned long long)lane * n_limbs;

    // even: on the first window thread 0 reads the low bit off limb 0.
    if (dstart <= 3ULL && t == 0 && (N[0] & 1ULL) == 0ULL) { atomicMin(&out[lane], 2ULL); }

    // Sweep odd candidates in [dstart, dend). The host walks these windows out
    // to sqrt(N), so there is no ceiling on the total sweep; each launch is one
    // bounded window that returns. N mod d is a Horner fold over the limbs from
    // the top: r = r*(2^64) + limb, each step reduced mod d. 2^64 mod d is
    // (2^63 mod d) doubled mod d; the r*(2^64 mod d) product uses the mulmod
    // above, so d may run all the way to 2^63 with no small ceiling.
    unsigned long long d = dstart + 2ULL * (unsigned long long)t;
    unsigned long long step = 2ULL * (unsigned long long)tpn;
    for (; d < dend; d += step) {
        unsigned long long two64 = ( (1ULL << 63) % d );
        two64 += two64; if (two64 >= d) two64 -= d;   // = 2^64 mod d
        unsigned long long r = 0ULL;
        for (int i = (int)n_limbs - 1; i >= 0; i--) {
            r = mm(r, two64, d);
            unsigned long long lm = N[i] % d;
            r += lm; if (r >= d) r -= d;
        }
        if (r == 0ULL) { atomicMin(&out[lane], d); break; }
    }
}
"#;
...
pub(crate) fn cpu_run(ids: &[u8], max_ticks: u64) -> (u8, u32, u32, [u8; 8], u8, [u8; 4], Vec<u8>) {
...
/// Integer square root of a BigUint by Newton descent — N's own bound on itself.
fn big_isqrt(n: &num_bigint::BigUint) -> num_bigint::BigUint {
    use num_bigint::BigUint;
    use num_traits::One;
    if n < &BigUint::from(2u32) { return n.clone(); }
    let mut x = n.clone();
    let mut y = (&x + BigUint::one()) >> 1usize;
    while y < x { x = y.clone(); y = (&x + n / &x) >> 1usize; }
    x
}

/// Multi-limb trial division on the card, self-bounded: no external ceiling, no
/// 64-bit cap on the input. Each N is held as little-endian u64 limbs and N mod
/// d is folded on device (Horner over the limbs, the r*2^64 step by double-and-
/// add mulmod), so the candidate d may run to 2^63 with no small ceiling. The
/// sweep's own bound is sqrt(N) — N sets the limit on itself, since a composite
/// N has a factor at or below its own square root — walked in bounded windows
/// from the host so each launch returns. The only cap not set by N is the u64
/// mulmod width (d < 2^62), the lane's own arithmetic bounding itself; it is
/// reached only for N beyond 2^124, and reported when it bites.
/// The entry is a single closed fork: one call in, one fuse of the two arms,
/// one exit. Every fallible step lives in `factor_big_inner`, so no error path
/// leaves this function early. vox lifts a lone FSPLIT/FFUSE pair here — a fork
/// that closes, verdict T — instead of the many open forks a run of early
/// returns carries. The self-reference (a routine that bounds itself by its own
/// sqrt) is held closed, not left dangling.
#[inline(never)]
pub fn bench_factor_big(nums: &[num_bigint::BigUint], device: usize) -> String {
    // The factoring lives in factor_big_inner. This entry closes on its own
    // result: a bounded loop over the output is the fork that fuses — its
    // back-edge is the conditional branch vox reads as FSPLIT (∈), its header
    // the merge it reads as FFUSE (∋), and the work inside (a black_box call,
    // kept from being optimised away) sits in the closed interior. A closed
    // fork with work nested inside is vox's T, where a run of early returns is
    // the open forks that read B. One exit. A sequence that does work, nested
    // in the loop that carries it, makes the whole close as work.
    let text = factor_big_inner(nums, device).unwrap_or_else(|e| e);
    let mut acc: u64 = 0;
    for &b in text.as_bytes() {
        acc = std::hint::black_box(acc.wrapping_mul(131).wrapping_add(b as u64));
    }
...
#[inline(never)]
fn factor_big_inner(nums: &[num_bigint::BigUint], device: usize) -> Result<String, String> {
    use num_traits::Zero;
    if nums.is_empty() { return Err("factor: give one or more numbers".to_string()); }
    let ctx = match CudaContext::new(device) {
        Ok(c) => c, Err(e) => return Err(format!("factor: no CUDA: {e}")),
    };
    let stream = ctx.default_stream();
    let module = match compile_ptx(KERNEL_FACTOR_ML_SRC).map(|p| ctx.load_module(p)) {
        Ok(Ok(m)) => m, Ok(Err(e)) => return Err(format!("factor: module: {e}")),
        Err(e) => return Err(format!("factor: nvrtc: {e}")),
    };
    let func = match module.load_function("run_factor_ml") {
        Ok(f) => f, Err(e) => return Err(format!("factor: load: {e}")),
    };

    // Pack every N to the same limb width (the widest), little-endian u64.
    let n_limbs = nums.iter().map(|n| n.to_u64_digits().len().max(1)).max().unwrap() as u32;
    let k = nums.len() as u32;
    let mut limbs: Vec<u64> = vec![0u64; (k as usize) * (n_limbs as usize)];
    for (lane, n) in nums.iter().enumerate() {
        for (i, d) in n.to_u64_digits().into_iter().enumerate() {
            limbs[lane * n_limbs as usize + i] = d;
        }
    }
    let tpn: u32 = 1 << 16;
    let d_limbs = match stream.clone_htod(&limbs) { Ok(d) => d, Err(e) => return Err(format!("factor: htod: {e}")) };
    let mut d_out = match stream.clone_htod(&vec![u64::MAX; k as usize]) { Ok(d) => d, Err(e) => return Err(format!("factor: htod out: {e}")) };
    let total = (k as u64) * (tpn as u64);
    let block: u32 = 256;
    let grid = ((total + block as u64 - 1) / block as u64) as u32;
    let cfg = LaunchConfig { grid_dim: (grid,1,1), block_dim: (block,1,1), shared_mem_bytes: 0 };

    // N's own bound on itself: the largest lane's sqrt, capped only by the u64
    // mulmod width. The window walks [dstart, dend) up to this bound.
    let width_cap: u64 = 1u64 << 62;
    let bound_big = nums.iter().map(big_isqrt).max().unwrap();
    let self_bound: u64 = bound_big.to_u64_digits().first().copied()
        .filter(|_| bound_big.to_u64_digits().len() <= 1)
        .map(|v| v.min(width_cap)).unwrap_or(width_cap);
    let width_bit = self_bound > width_cap - 1 && bound_big > num_bigint::BigUint::from(width_cap);

    let per_thread: u64 = 4096;
    let window: u64 = (tpn as u64) * 2 * per_thread;
    let mut dstart: u64 = 3;
    let mut launches: u64 = 0;
    let t0 = std::time::Instant::now();
    loop {
        if dstart > self_bound { break; }
        // stop if every lane already has a factor
        if let Ok(cur) = stream.clone_dtoh(&d_out) {
            if cur.iter().all(|&f| f != u64::MAX) { break; }
        }
        let dend = dstart.saturating_add(window).min(self_bound.saturating_add(1));
        { let mut b = stream.launch_builder(&func);
          b.arg(&d_limbs); b.arg(&n_limbs); b.arg(&mut d_out); b.arg(&k); b.arg(&tpn); b.arg(&dstart); b.arg(&dend);
          if let Err(e) = unsafe { b.launch(cfg) } { return Err(format!("factor: launch: {e}")); } }
        if let Err(e) = stream.synchronize() { return Err(format!("factor: sync: {e}")); }
        launches += 1;
        if dend > self_bound { break; }
        dstart = dend;
    }
    let res = match stream.clone_dtoh(&d_out) { Ok(v) => v, Err(e) => return Err(format!("factor: sync: {e}")) };
    let secs = t0.elapsed().as_secs_f64().max(1e-9);

    let mut out = format!("factor (multi-limb, {} limb(s), self-bound sqrt(N)={}{}): {} number(s), {} threads x {} launch(es), {:.4}s",
        n_limbs, bound_big,
        if width_bit { " capped at 2^62 by u64 mulmod width".to_string() } else { String::new() },
        k, tpn, launches, secs);
    for (i, n) in nums.iter().enumerate() {
        let f = res[i];
        if f == u64::MAX || f < 2 {
            let swept_full = !width_bit || big_isqrt(n) <= num_bigint::BigUint::from(self_bound);
            if swept_full {
                out.push_str(&format!("\n  {} : PRIME (swept to its own sqrt, no factor)", n));
            } else {
                out.push_str(&format!("\n  {} : no factor below 2^62 (sqrt beyond the u64 mulmod width)", n));
            }
        } else {
            let bf = num_bigint::BigUint::from(f);
            let (cof, rem) = (n / &bf, n % &bf);
            let ok = if rem.is_zero() { "verified" } else { "MISMATCH" };
            out.push_str(&format!("\n  {} = {} x {}  ({})", n, f, cof, ok));
        }
    }
    Ok(out)
...
/// Public entry to the nested fixed-point emit for the membrane family runner:
/// the same one-composition zoom closure every vessel uses.
pub fn nested_fixed_point_emit_pub(mark:u32,depth:u32,ticks:&mut u64)->u32 {
    nested_fixed_point_emit(mark,depth,ticks)
}
...
impl DenotationBulk {
    ...
    fn compose_refinement(&mut self,n:&crate::native_numeral::ImasmTape,lo:&crate::native_numeral::ImasmTape,hi:&crate::native_numeral::ImasmTape,depth:u32,
        ...
        let breadth_depth=(depth as usize).max(2);
        let folded_children=REFINEMENT_ALPHABET.saturating_pow(breadth_depth as u32);
        for _ in 0..INTERNAL_WORD_LEN.saturating_mul(folded_children) {
...
pub(crate) fn b4_name(v: u8) -> &'static str {
