...
pub static TOOLS_MENU: &[MenuItem] = &[
    ...
    MenuItem { name: "imasm_mul", cmd: "imasm_mul", desc: "Compile a width-specialized schoolbook multiplier whose partial products and carries execute as IMASM", example: "imasm_mul ⊥⊥ ⊥⊥", submenu: None },
    MenuItem { name: "imasm_divmod", cmd: "imasm_divmod", desc: "Compile restoring long division as IMASM; returns encoded quotient and remainder", example: "imasm_divmod ⊥⊥⊥⊥ ⊥⊥", submenu: None },
    MenuItem { name: "imasm_mod", cmd: "imasm_mod", desc: "Return the remainder from the IMASM restoring-division circuit", example: "imasm_mod ⊥⊥⊥⊥ ⊥⊥", submenu: None },
    MenuItem { name: "imasm_gcd", cmd: "imasm_gcd", desc: "Compose restoring division into a width-specialized Euclidean closure, entirely in IMASM", example: "imasm_gcd ⊥⊥⊥⊥ ⊥⊤⊥", submenu: None },
    MenuItem { name: "imasm_close", cmd: "imasm_close", desc: "Nest IMASM multiplication inside exact product-to-N closure; all three operands are encoded tapes", example: "imasm_close ⊥⊥ ⊥⊤⊥ ⊥⊥⊥⊥", submenu: None },
    MenuItem { name: "imasm_edit_square", cmd: "imasm_edit_square", desc: "Check the token-edit commuting square and its additive/multiplicative closure in one dynamically sized IMASM stream", example: "imasm_edit_square ⊤⊥ ⊤⊥", submenu: None },
    MenuItem { name: "imasm_powmod", cmd: "imasm_powmod", desc: "Compose phase-bit branches, IMASM multiplication, and restoring modulus closure into a modular-winding membrane", example: "imasm_powmod ⊤⊥ ⊤⊥⊤⊥ ⊥⊤⊤⊤⊥", submenu: None },
    ...
    MenuItem { name: "shor_b4", cmd: "shor_b4", desc: "Key membrane finding: shors_algorithm x b4_factor_v2_engine click (T<->H, certified) + membrane family (aliases: shor_b4_membrane, key_membrane)", example: "shor_b4 click", submenu: None },
    MenuItem { name: "membrane_family", cmd: "membrane_family", desc: "Membrane family registry: list/run factor membranes by word (alias: mfam)", example: "membrane_family list", submenu: None },
    MenuItem { name: "doubly_nested_oneshot",      cmd: "doubly_nested_oneshot",     desc: "Winding-order factoring by hypernest depth: each depth wraps one more carrier and gives a fresh Brent seed. factor <N> [maxdepth] sweeps the ladder; ladder <N> [maxdepth] shows every depth's word, seed and budget (alias: dnos)", example: "doubly_nested_oneshot ladder 3215031751", submenu: None },
    ...
    MenuItem { name: "shiab",                      cmd: "shiab",                     desc: "SHIABO: holographic scale-collapse. An integer collapses to its δ boundary with μ∘δ=id recovery and winding; an IMASM word is wrapped as bulk in ⊢⊙∈…⋈∋⊡⊣ and run", example: "shiab 42", submenu: None },
    MenuItem { name: "hyperstack",                 cmd: "hyperstack",                desc: "Heterogeneous hypernesting: load a value through a stack of different carrier types (phase, shor, fib, ...), each carrier's word the payload of the next; reads register, verdict and winding at each layer. Phase closes to T, Shor opens the both-value B, Fibonacci holds it", example: "hyperstack 91 phase shor fib", submenu: None },
...
pub static GRAMMAR_MENU: &[MenuItem] = &[
    ...
    MenuItem { name: "millennium", cmd: "millennium", desc: "run weight | banked | insert on a Millennium conjecture's promotion word and print its live executed crystal address and tuple; no argument lists the seven names, 'all' runs every one", example: "millennium rh_positivity_promotion", submenu: None },
];
