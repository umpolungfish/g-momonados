...
hosted:
	...
	@cp target/$(HOST)/debug/$(BIN) target/release/$(BIN)
	@echo "hosted binary synced to target/release/$(BIN) for qr3/run_quit.sh"
...
ordinals: hosted
	@timeout 60 ./target/$(HOST)/debug/$(BIN) < /dev/null 2>&1 \
	   > .ordinals.log || true
